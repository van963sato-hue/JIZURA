...
7. **MP4** または **連番PNG** で、編集した映像と文字を合成して書き出します。尺は編集後の全クリップの合計です。**透過PNG** は文字とその演出だけを書き出すので、別の動画編集ソフトでも重ねられます。MP4 書き出しには WebCodecs と対応エンコーダーが必要です（Chrome / Edge 推奨）。
...
**保存と再開：** プロジェクト JSON には歌詞・タイミング・MV 設定・クリップの順番と編集値・素材のファイル情報を保存します。動画や音声ファイル本体は含みません。開き直したら同じ動画ファイルを再選択してください。保存したファイル情報に一致する素材は既存のクリップに再接続され、複製・分割したクリップも復元されます。別の曲を使う場合は、その音声ファイルも再選択します。MV 編集・合成はブラウザ版の機能で、AE パネルへの動画編集の転送には対応していません。
