...
### 手持ちの MV に文字アニメーションを重ねる（ブラウザ版）

1. **MVに文字を重ねる** で動画ファイルを読み込みます。MP4 / WebM など、使用中のブラウザで再生できる形式・コーデックに対応します。動画はブラウザ内で処理され、アップロードされません。
2. 歌詞を入力し、各行の **開始・終了（秒）** を指定します。LRC のタイムスタンプやタップ同期も使えます。LRC を読み込んだあとでも手動の時刻を優先して調整でき、終了時刻を早めれば間奏などに文字の出ない区間を作れます。
3. 新しい動画を読み込むと、その動画の画面比を自動で採用します。別の画面比で書き出す場合は書き出し設定で変更し、**全体を収める / 画面いっぱい** で収まり方を選びます。中央・下・上の配置、文字の大きさ・左右位置・上下位置・影、動画を暗くする量も調整できます。
4. 音声は **元動画の音声 / 別に読み込んだ曲 / 音声なし** から選びます。「別に読み込んだ曲」は別の音声ファイルへの差し替えです。動画の音声をブラウザで取り出せない場合は、音声を別に読み込むか、明示的に「音声なし」を選んでください。
5. **MP4** または **連番PNG** で、動画と文字を合成した状態を書き出します。尺は元動画の長さです。**透過PNG** は文字とその演出だけを書き出すので、別の動画編集ソフトでも重ねられます。MP4 書き出しには WebCodecs と対応エンコーダーが必要です（Chrome / Edge 推奨）。

MV モードでは元の文字アニメーションと配色を使い、映像を覆うスタイル背景・HUD・画面全体の仕上げ効果・カット間のつなぎを省きます。自動タイトルカードや自動の間奏カットも挿入しません。既存のレイアウトには帯やパネルなどが含まれるため、映像の見せたい部分に合わせてレイアウトも選んでください。

**保存と再開：** プロジェクト JSON には歌詞・タイミング・MV 設定・動画のファイル名を保存します。動画や音声ファイル本体は含みません。プロジェクトを開き直したら、同じ動画と必要な音声ファイルを再選択してください。MV 合成はブラウザ版の機能です。
